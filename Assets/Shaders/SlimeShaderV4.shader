Shader "Custom/SlimeShaderV2"
{
    Properties
    {
        ...
        _NoiseScale ("Noise Scale", Range(0.5, 5)) = 2.0
        _FlowSpeed ("Flow Speed", Range(0, 3)) = 0.7
        _BoilAmount ("Boil Amount", Range(0, 0.25)) = 0.08
        _BoilSpeed ("Boil Speed", Range(0, 5)) = 2.0

        _HitBoost ("Facade Hit Boost", Range(0, 1)) = 0.5
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            // Smoothed sampling som respekterar obstacles
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // --- NOISE-WOBBLE BARA PÅ FÄRG ---
                float2 nUv = uv * _NoiseScale + float2(_Time.y * _FlowSpeed, 0);
                float n = tex2D(_NoiseTex, nUv).r;            // 0..1
                float wobble = (n - 0.5) * 2.0;               // -1..1
                float boilMask = thickness * thickness;       // starkare i tjockt slime
                float wobbleAmount = _BoilAmount;

                col.rgb *= 1.0 + wobble * wobbleAmount * boilMask *
                                 (0.5 + 0.5 * sin(_Time.y * _BoilSpeed));

                // rim + kantglow
                col.rgb = lerp(col.rgb, _EdgeColor.rgb, fresnel);
                col.rgb += _EdgeColor.rgb * edge * 0.3;

                // extra pop där slime ligger på kanten av obstacles (topp av fönster m.m.)
                float obsBelow = IsObstacle(uv - float2(0, _MainTex_TexelSize.y * 1.5));
                col.rgb = lerp(col.rgb, _EdgeColor.rgb * 1.5, obsBelow * _HitBoost * edge);
