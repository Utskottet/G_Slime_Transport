Shader "Custom/SlimeShaderV2"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Smooth sampling - average nearby pixels for softer edges
            float SampleSmooth(float2 uv, float radius) {
                ...
                for (int x = -2; x <= 2; x++) {
                    for (int y = -2; y <= 2; y++) {
                        ...
                        sum += tex2D(_MainTex, uv + offset).r * weight;
                        total += weight;
                    }
                }
                return sum / total;
            ...
            fixed4 frag (v2f i) : SV_Target {
                // Animated UV for subtle wobble
                float2 uv = i.uv;
                uv += sin(_Time.y * _PulseSpeed + uv.yx * 10) * _PulseAmount;
